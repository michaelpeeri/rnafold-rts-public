    ...
    def performPlots(self):
        ...
        for shuffleType in shuffleTypes:
                ...
                if (not oinf is None) and (oinf[1] > 1): # only include operonic genes
                    ...
                    if oinf[0]==oinf[1]-1:
                        ...
                        if not utr3Length is None:
                            allRealUtr3Lengths.append( utr3Length )
                        else:
                            continue
            ...
            if args.plot_adaptive_lengths:
                numGroups = 10
                positiveLengths = [x for x in sorted(allRealUtr3Lengths) if x>=0]
                ...
                groups.append(max(allRealUtr3Lengths))  # The last group contains all items left
                ...
                for result in allCDSrecords:
                    ...
                    if (not oinf is None) and (oinf[1] > 1): # only include operonic genes
                        if oinf[0]==oinf[1]-1: # last in operon
                            pass
                        else:
                            continue
                    else:
                    ...
                    if utr3Length<0:  # not possible...
                        h5destination_integenic_adaptive_neg.accumulate( profileData, seq, fullCDS )
                        actualGroupSizes[0] += 1
                    else:
                        matchingGroup = next( n for n,i in enumerate([-1]+groups) if i>=utr3Length ) # Find group to hold item; return 1-based group index
