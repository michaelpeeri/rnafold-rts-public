...
from mfe_plots import plotMFEProfile, plotMFEProfileV3, plotXY, scatterPlot, scatterPlotWithColor, plotMFEProfileByPA, plotMFEProfileMultiple, scatterPlotWithKernel, plotMFEProfileForMultipleRandomizations, plot2WayMFEComparison, plotMultipleMFEComparison, plot2dProfile, plot2dProfileAsRidgeplot, plotMultipleDestinations, plotSmoothedLengthEffectData, plotHistogramComparison, plotPAratioVsDistanceHeatmap, plotPAregression
...
class PlottingDestination(object):
    def __init__(self, args, shuffleType, taxid, subclass="all", wilcoxonDLFEZero=False, wilcoxonDLFE50_100=False, saveSequenceProfiles=False):
        self.args = args
        self.taxid = taxid
        ...
        self.saveSequenceProfiles = saveSequenceProfiles

        self.reset()

    def accumulate(self, profileData, seq, fullCDS, positions:dict={}, seqId=None):
        args = self.args
        ...
        if self.saveSequenceProfiles:
            assert(not seqId is None)
            
            if profileData.shape[1]==200:
                self.nativeLFEs = self.nativeLFEs.append( pd.DataFrame( dict(zip(range(-100,100), profileData[0,None].flat )), index=pd.Index([seqId]), columns=range(-100,100) ) )
                self.randomizedLFEs = self.randomizedLFEs.append( pd.DataFrame( dict(zip(range(-100,100), profileData[1:].mean(axis=0).flat )), index=pd.Index([seqId]), columns=range(-100,100) ) )
    ...
    def reset(self):
        ...
        if self.saveSequenceProfiles:
            #self.sequenceProfiles = np.zeros( shape=(0, self.profileLen), dtype=np.float32 ) )
            self.nativeLFEs = pd.DataFrame( columns=range(-100,100), index=pd.Index([]) )
            self.randomizedLFEs = pd.DataFrame( columns=range(-100,100), index=pd.Index([]) )
    ...
    def finalize(self):
        ...
        if self.calcWilcoxonDLFE50_100:
            startIdx = profileElements(args.profile).index(50)
            ...
            wilcoxonDLFE50_100 = pd.DataFrame({
                'pos':pd.Series(xRange, dtype='int'),
                'statistic':pd.Series(wilcoxonResults[0], dtype='int'),
                'pval':pd.Series(wilcoxonResults[1], dtype='float'),
                'N':pd.Series( np.repeat( dataForTest.shape[0], self.profileLen), dtype='int'),
                'sign':pd.Series(wilcoxonSigns, dtype='int')})
            print(wilcoxonDLFE50_100.shape)
        ...
        with pd.io.pytables.HDFStore(h5fn, complib="zlib", complevel=1) as store:
            ...
            if self.calcWilcoxonDLFE50_100:
                ...
                print("wilcoxonDLFE50_100")
                print(wilcoxonDLFE50_100)
                wilcoxonDLFE50_100.to_csv("wilcoxonDLFE50_100.csv")

            if self.saveSequenceProfiles:
                store["sequence_native_lfe_%d_%d_%d_%s_%d" % (taxid, args.profile[0], args.profile[1], args.profile[2], args.profile[3])]     = self.nativeLFEs
                store["sequence_randomized_lfe_%d_%d_%d_%s_%d" % (taxid, args.profile[0], args.profile[1], args.profile[2], args.profile[3])] = self.randomizedLFEs
...
def readSeriesResultsForSpeciesWithSequence_cached( seriesSourceNumber, species, minShuffledGroups, maxShuffledGroups, shuffleType, cdsFilter=None, returnCDS=True ):
    return list(readSeriesResultsForSpeciesWithSequence(seriesSourceNumber, species, minShuffledGroups, maxShuffledGroups, shuffleType, cdsFilter=cdsFilter, returnCDS=returnCDS ))


def printGroupComparisonTable( profile1, profile2 ):

    out = pd.DataFrame({'pos':pd.Series(dtype='int'),
                        'N1':pd.Series(dtype='int'),
                        'N2':pd.Series(dtype='int'),
                        'median1':pd.Series(dtype='float'),
                        'median2':pd.Series(dtype='float'),
                        'MannWhitneyU':pd.Series(dtype='float'),
                        'pvalue':pd.Series(dtype='float'),
                        'alternative':pd.Series(dtype='str') })


    def collectData(dfs):
        if type(dfs)==type(()):
            return '_'.join( [x.subclass for x in dfs]), np.concatenate( [x.wilcoxon for x in dfs] )
        else:
            return dfs.subclass, dfs.wilcoxon

    subclass1, profile1 = collectData( profile1 )
    subclass2, profile2 = collectData( profile2 )
        
    for pos in range(0, profile1.shape[1]):
        d1 = profile1[:,pos]
        d2 = profile2[:,pos]
        test = mannwhitneyu( d1, d2, alternative='two-sided' )
        out = out.append( pd.DataFrame({'pos':pd.Series([pos],                     dtype='int'),
                                        'N1':pd.Series([d1.shape[0]/20],           dtype='int'),
                                        'N2':pd.Series([d2.shape[0]/20],           dtype='int'),
                                        'median1':pd.Series([np.median(d1)],       dtype='float'),
                                        'median2':pd.Series([np.median(d2)],       dtype='float'),
                                        'MannWhitneyU':pd.Series([test.statistic], dtype='float'),
                                        'pvalue':pd.Series([test.pvalue],          dtype='float'),
                                        'alternative':pd.Series(['two-sided'],     dtype='str')}) )

        #print("{},{},{},{},{},two-sided".format( pos, d1.shape[0]/20, d2.shape[0]/20, test.statistic, test.pvalue ))
    out.to_csv('mfe_v2_mann_whitney_{}_{}.csv'.format( subclass1, subclass2 ))
...
class ProfilePlot(object):
    ...
    def performPlots(self):
        ...
        for shuffleType in shuffleTypes:
            ...
            h5destination     = PlottingDestination(args, shuffleType, taxid, wilcoxonDLFEZero=True, wilcoxonDLFE50_100=True, saveSequenceProfiles=True)
            ...
            h5destination_next_cds_same     = PlottingDestination(args, shuffleType, taxid, subclass="next_cds_same", wilcoxonDLFEZero=True)
            h5destination_next_cds_opposite = PlottingDestination(args, shuffleType, taxid, subclass="next_cds_opposite", wilcoxonDLFEZero=True)
            ...
            h5destination_integenic_negative = PlottingDestination(args, shuffleType, taxid, subclass="intergenic_negative", wilcoxonDLFEZero=True)
            h5destination_integenic_0_19     = PlottingDestination(args, shuffleType, taxid, subclass="intergenic_0_19",     wilcoxonDLFEZero=True)
            ...
            h5destination_integenic_80_999   = PlottingDestination(args, shuffleType, taxid, subclass="intergenic_80_999", wilcoxonDLFEZero=True)
            ...
            h5destination_operon_last        = PlottingDestination(args, shuffleType, taxid, subclass="operon_last", wilcoxonDLFEZero=True)
            h5destination_operon_last_and_not_in_ext_class \
                                             = PlottingDestination(args, shuffleType, taxid, subclass="operon_last_not_in_ext")
            h5destination_operon_not_last    = PlottingDestination(args, shuffleType, taxid, subclass="operon_not_last", wilcoxonDLFEZero=True)
            
            h5destination_operon_not_last_aug    = PlottingDestination(args, shuffleType, taxid, subclass="operon_not_last_with_aug")
            h5destination_operon_not_last_and_not_in_ext_class \
                                             = PlottingDestination(args, shuffleType, taxid, subclass="operon_not_last_not_in_ext")
            ...
            h5destination_pa_low          = PlottingDestination(args, shuffleType, taxid, subclass="pa_low", wilcoxonDLFEZero=True)
            h5destination_pa_med          = PlottingDestination(args, shuffleType, taxid, subclass="pa_med", wilcoxonDLFEZero=True)
            h5destination_pa_high         = PlottingDestination(args, shuffleType, taxid, subclass="pa_high", wilcoxonDLFEZero=True)
            ...
            h5destination_external_class_in  = PlottingDestination(args, shuffleType, taxid, subclass="ext_class_in")
            h5destination_external_class_out = PlottingDestination(args, shuffleType, taxid, subclass="ext_class_out")
            bdqaSetsExternalClass = {}
            ...
            if args.dump:
                dfGeneLevelExport = pd.DataFrame({'taxId':pd.Series(dtype='int'),
                                                  '3UTR_intergenic_length_nt':pd.Series(dtype='int'),
                                                  'PA':pd.Series(dtype='float'),
                                                  'dLFE_pos_-50':pd.Series(dtype='float'),
                                                  'dLFE_pos_-47':pd.Series(dtype='float'),
                                                  'dLFE_pos_-44':pd.Series(dtype='float'),
                                                  'dLFE_pos_-41':pd.Series(dtype='float'),
                                                  'dLFE_pos_-38':pd.Series(dtype='float'),
                                                  'dLFE_pos_-35':pd.Series(dtype='float'),
                                                  'dLFE_pos_-32':pd.Series(dtype='float'),
                                                  'dLFE_pos_-29':pd.Series(dtype='float'),
                                                  'dLFE_pos_-26':pd.Series(dtype='float'),
                                                  'dLFE_pos_-23':pd.Series(dtype='float'),
                                                  'dLFE_pos_-20':pd.Series(dtype='float'),
                                                  'dLFE_pos_-17':pd.Series(dtype='float'),
                                                  'dLFE_pos_-14':pd.Series(dtype='float'),
                                                  'dLFE_pos_-11':pd.Series(dtype='float'),
                                                  'dLFE_pos_-8':pd.Series(dtype='float'),
                                                  'dLFE_pos_-5':pd.Series(dtype='float'),
                                                  'dLFE_pos_-2':pd.Series(dtype='float'),
                                                  'dLFE_pos_1':pd.Series(dtype='float'),
                                                  'dLFE_pos_4':pd.Series(dtype='float'),
                                                  'dLFE_pos_7':pd.Series(dtype='float'),
                                                  'dLFE_pos_10':pd.Series(dtype='float'),
                                                  'dLFE_pos_13':pd.Series(dtype='float'),
                                                  'dLFE_pos_16':pd.Series(dtype='float'),
                                                  'dLFE_pos_19':pd.Series(dtype='float'),
                                                  'dLFE_pos_22':pd.Series(dtype='float'),
                                                  'dLFE_pos_25':pd.Series(dtype='float'),
                                                  'dLFE_pos_28':pd.Series(dtype='float'),
                                                  'dLFE_pos_31':pd.Series(dtype='float'),
                                                  'dLFE_pos_34':pd.Series(dtype='float'),
                                                  'dLFE_pos_37':pd.Series(dtype='float'),
                                                  'dLFE_pos_40':pd.Series(dtype='float'),
                                                  'dLFE_pos_43':pd.Series(dtype='float'),
                                                  'dLFE_pos_46':pd.Series(dtype='float'),
                                                  'dLFE_pos_49':pd.Series(dtype='float'),
                                                  'operon_position':pd.Series(dtype='int'),
                                                  'operon_length':pd.Series(dtype='int'),
                                                  'this_cds_strand':pd.Series(dtype='str'),
                                                  'next_cds_strand':pd.Series(dtype='str')
                })
            ...
            externalClassMembers = set()
            if args.external_class:
                externalClassMembers = loadExternalSet( args.external_class )
            ...
            allCDSrecords = list( sampleProfilesFixedIntervals(
                convertResultsToMFEProfiles(
                    #readSeriesResultsForSpeciesWithSequence_cached((args.computation_tag,), taxid, numShuffledGroups, numShuffledGroups, shuffleType=shuffleType )
                    readSeriesResultsForSpeciesWithSequence_cached((args.computation_tag,), taxid, numShuffledGroups, numShuffledGroups, shuffleType=shuffleType )
                    , numShuffledGroups)
            ...
            for result in allCDSrecords:
                ...
                h5destination.accumulate( profileData, seq, fullCDS, positions={'stop-codon':stopCodonPos, '3utr-length':flanking3UTRRegionLengthNt }, seqId=protId )
                ...
                if args.external_class:
                    if protId in externalClassMembers:
                        h5destination_external_class_in.accumulate( profileData, seq, fullCDS )
                        bdqaSetsExternalClass[ protId ] = "ext class"
                    else:
                        h5destination_external_class_out.accumulate( profileData, seq, fullCDS )

                gid = cds.getGeneId()
                oinf = cds.getOperonInfo()

                if args.dump:
                    dfGeneLevelExport.loc[gid, "taxId"] = taxid
                    dfGeneLevelExport.loc[gid, "3UTR_intergenic_length_nt"] = flanking3UTRRegionLengthNt
                    
                    if profileData.shape[1] == 200:
                        for pos in range(-50,52,3):
                            dfGeneLevelExport.loc[gid, "dLFE_pos_{}".format(pos)] = profileData[0, 100+pos] - np.mean(profileData[1:, 100+pos])
                ...
                if (not oinf is None):
                    ...
                    if args.dump:
                        dfGeneLevelExport.loc[gid, "operon_position"] = operonPosFromStart
                        dfGeneLevelExport.loc[gid, "operon_length"]   = oinf[1]
                    ...
                    if operonPosFromEnd==0:
                        ...
                        if (not args.external_class) or \
                           (args.external_class and not protId in externalClassMembers):
                                h5destination_operon_last_and_not_in_ext_class.accumulate( profileData, seq, fullCDS )
                                bdqaSetsExternalClass[protId] = "last and not in ext class"
                        ...
                        if (not args.external_class) or \
                           (args.external_class and not protId in externalClassMembers):
                                h5destination_operon_not_last_and_not_in_ext_class.accumulate( profileData, seq, fullCDS )
                                bdqaSetsExternalClass[protId] = "not last + not in ext class"
                ...
                if args.dump:
                    dfGeneLevelExport.loc[gid, "this_cds_strand"] = strand
                    if strand=='+':
                        if nextCDSOnOppositeStrand:
                            nextCDSStrand = '-'
                        else:
                    ...
                    dfGeneLevelExport.loc[gid, "next_cds_strand"] = nextCDSStrand
                ...
                if flanking3UTRRegionLengthNt >= 25:
                ...
                if args.pa:
                    ...
                    if not PA is None:
                        ...
                        if args.dump:
                            dfGeneLevelExport.loc[gid, "PA"] = PArel
                ...
                #if n > 500: break  # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY 
                # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY # DEBUG ONLY 
            ...
            if args.pa:
                printGroupComparisonTable( h5destination_pa_high, (h5destination_pa_med, h5destination_pa_low) )
            ...
            if args.dump:
                dfGeneLevelExport.to_csv('mfe_v2_gene_level_export_{}_{}_{}.csv'.format( taxid, profileId, shuffleType))
            ...
            plotMFEProfile(taxid, profileId, df, computationTag=args.computation_tag, yRange=(-8.5,0.2), showRandomizedRange=False)
            plotMFEProfile(taxid, profileId, df, computationTag=args.computation_tag,                    showRandomizedRange=True, fileLabel="withrand" )
            plotMFEProfile(taxid, profileId, df, computationTag=args.computation_tag, yRange=(-4,0.5),   showRandomizedRange=False, showComponents=False,  fileLabel="snapshot", aspect=40.0, tickValues = range(-100,101,50), linewidth=3.5 )
            ...
            # Plot: Intergenic distance, Short vs. Long vs. Overlapping
            printGroupComparisonTable( h5destination_integenic_80_999, (h5destination_integenic_negative, h5destination_integenic_0_19) )
            plotMultipleDestinations( taxid, "utr3prime_length", (h5destination_integenic_negative, h5destination_integenic_0_19, h5destination_integenic_80_999), ((-30,0),(0,24),(25,999)), xRange )
            ...
            printGroupComparisonTable( h5destination_next_cds_same, h5destination_next_cds_opposite)
            ...
            numGenesStopCodonUAA     = np.min(h5destination_taa.nativeMeanProfile.counts())
            numGenesStopCodonUGA     = np.min(h5destination_tga.nativeMeanProfile.counts())
            numGenesStopCodonUAG     = np.min(h5destination_tag.nativeMeanProfile.counts())
            plotMultipleMFEComparison( taxid,
                                       profileId,
                                       (
                                           (pd.DataFrame({'native':   h5destination_taa.getNativeProfile(),
                                                          'shuffled': h5destination_taa.getShuffledProfile()},
                                                         index=xRange),
                                            "UAA"),
                                           (pd.DataFrame({'native':   h5destination_tga.getNativeProfile(),
                                                          'shuffled': h5destination_tga.getShuffledProfile()},
                                                         index=xRange),
                                            "UGA"),
                                           (pd.DataFrame({'native':   h5destination_tag.getNativeProfile(),
                                                          'shuffled': h5destination_tag.getShuffledProfile()},
                                                         index=xRange),
                                            "UAG"),
                                       ),
                                       computationTag=args.computation_tag,
                                       ...
                                       aspect=25.0,
                                       yRange=(-5.1,1.1),
                                       yTickValues=range(-5,2,1),
                                       xTickValues=range(-80,81,20),
                                       comment="UAA (N={}) UGA (N={}) UAG (N={}) Total (N={}) ".format( numGenesStopCodonUAA, numGenesStopCodonUGA, numGenesStopCodonUAG, numGenesStopCodonUGA+numGenesStopCodonUAG+numGenesStopCodonUAA ),
                                       plotname="comparison_stop_codon")
            ...
            printGroupComparisonTable( h5destination_operon_last, h5destination_operon_not_last )
            plotMultipleMFEComparison( taxid,
                                   profileId,
                                       (
            ...
            plotMultipleMFEComparison( taxid,
                                   profileId,
                                       ...
                                       computationTag=args.computation_tag,
                                       showComponents=False,
                                       xTickValues=range(-100,101,50),
                                       yRange=(-8.5,1.0),
                                       yTickValues=range(-8,1,2),
                                       lineWidth=4,
                                       aspect=20.0,
                                       comment="",
                                       showLegend=False,
                                       plotname="comparison_operon_position_snapshot",
                                    destinations=(h5destination_operon_last, h5destination_operon_1st_and_not_last, h5destination_operon_2nd_and_not_last, h5destination_operon_3rd_and_not_last ))
            ...
            if args.external_class:
                numGenesInExternalClass    = np.min(h5destination_external_class_in.nativeMeanProfile.counts())
                #numGenesOutExternalClass   = np.min(h5destination_external_class_out.nativeMeanProfile.counts())
                numGenesNotLastOrInExtClass= np.min(h5destination_operon_not_last_and_not_in_ext_class.nativeMeanProfile.counts())
                numGenesLastInOperon       = np.min(h5destination_operon_last_and_not_in_ext_class.nativeMeanProfile.counts())

                plotMultipleMFEComparison( taxid,
                                       profileId,
                                           (
                                               (pd.DataFrame({'native':   h5destination_external_class_in.getNativeProfile(),
                                                              'shuffled': h5destination_external_class_in.getShuffledProfile()},
                                                             index=xRange),
                                                "In"),
                                               (pd.DataFrame({'native':   h5destination_operon_not_last_and_not_in_ext_class.getNativeProfile(),
                                                              'shuffled': h5destination_operon_not_last_and_not_in_ext_class.getShuffledProfile()},
                                                             index=xRange),
                                                "Out"),
                                               (pd.DataFrame({'native':   h5destination_operon_last_and_not_in_ext_class.getNativeProfile(),
                                                              'shuffled': h5destination_operon_last_and_not_in_ext_class.getShuffledProfile()},
                                                             index=xRange),
                                                "(Last or single) and not in class"),
                                           ),
                                       computationTag=args.computation_tag,
                                       showComponents=False,
                                       yRange=(-13,1),
                                       comment="In class (N={}) Out of class (N={}) Last or single (N={}) Total (N={}) ".format( numGenesInExternalClass, numGenesNotLastOrInExtClass, numGenesLastInOperon, numGenesInExternalClass+numGenesNotLastOrInExtClass+numGenesLastInOperon ),
                                       plotname="comparison_external_class")
        ...
        print("BDQA sets - ext class")
        print("------------------------------")
        counts_bdqa_ext = {}
        counts_bdqa_ext['total unique'] = 0
        for k, v in bdqaSetsExternalClass.items():
            if v in counts_bdqa_ext:
                counts_bdqa_ext[v] += 1
            else:
                counts_bdqa_ext[v]  = 1
            counts_bdqa_ext['total unique'] += 1
        print("Unique counts: {}".format( counts_bdqa_ext ))
...
if __name__=="__main__":
    ...
    argsParser.add_argument("--external-class", type=str, default=None)
